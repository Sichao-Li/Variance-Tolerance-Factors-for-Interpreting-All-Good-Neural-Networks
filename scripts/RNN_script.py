...
for i in range(150):
    ...
    feature_model.fit(X_train.astype(int), y_train, epochs=50, batch_size=64, verbose=0, callbacks=[stop_call_back])
    result = feature_model.layers[1].get_weights()[0]
...
#     with Pool(2) as p:
#         res = p.map(model_training, range(2))
#         np.save('./RNN_weights_output_1', np.array(res))
